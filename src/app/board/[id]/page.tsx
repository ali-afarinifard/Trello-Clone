...
export default function BoardPage() {
  ...
  if (!board) {
    return (
      <div className={styles.page} style={{ background: '#1a1f2e' }}>
        <Navbar />
        <main className={styles.notFound}>
          ...
          <p className={styles.notFoundText}>This board doesn&apos;t exist or has been deleted.</p>
          <a href="/" className={styles.backLink}>← Back to boards</a>
        </main>
      </div>
    );
  }

  const bg = BOARD_COLORS.find((c) => c.key === board.color)?.value ?? BOARD_COLORS[0].value;
